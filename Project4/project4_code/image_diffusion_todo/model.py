...
class DiffusionModule(nn.Module):
    ...
    def sample(
        ...
        if do_classifier_free_guidance:
            ...
            # Create null condition labels (zeros) for the first half
            null_labels = torch.zeros_like(class_label)
            # Concatenate null labels and class labels to create a tensor of shape (2*batch_size,)
            class_label = torch.cat([null_labels, class_label], dim=0)
            #######################
        ...
        for t in tqdm(self.var_scheduler.timesteps):
            ...
            if do_classifier_free_guidance:
                ...
                # Get noise predictions for both unconditional and conditional paths
                noise_pred_double = self.network(
                    x_t_double,
                ...
                # Apply classifier-free guidance formula: noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)
